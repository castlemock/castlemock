...
<c:choose>
    <c:when test="${soapMockResponses.size() > 0}">
        ...
        <form:form action="${delete_soap_mock_responses_url}" method="POST" commandName="deleteSoapMockResponsesCommand">
            ...
            <a href="<c:url value="/web/soap/project/${soapProjectId}/port/${soapPortId}/operation/${soapOperationId}"/>" class="button-secondary pure-button"><i class="fa fa-check-circle"></i> <spring:message code="soap.deletesoapmockresponses.button.cancel"/></a>
        </form:form>
    </c:when>
    <c:otherwise>

...
<div class="upload-file">
    <form:form method="POST" enctype="multipart/form-data" action="${import_url}" modelAttribute="uploadForm">
        ...
        <button class="button-success pure-button" name="type" value="file"><i class="fa fa-upload"></i> <span><spring:message code="rest.restimportdefinition.button.uploadfiles"/></span></button>
        <a href="<c:url value="/web/rest/project/${restProjectId}"/>" class="button-secondary pure-button"><i class="fa fa-times"></i> <spring:message code="rest.restimportdefinition.button.cancel"/></a>
        <input type="hidden" name="definitionType" value="${definitionType}"/>
        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
    </form:form>
</div>

...
<c:choose>
    <c:when test="${restMethods.size() > 0}">
        ...
        <form:form action="${delete_methods_url}" method="POST" commandName="deleteRestMethodsCommand">
            ...
            <button class="btn btn-danger" type="submit"><i class="fa fa-trash"></i> <span><spring:message code="rest.deletemethods.button.deletemethods"/></span></button>
            <a href="<c:url value="/web/rest/project/${restProjectId}/application/${restApplicationId}"/>" class="btn btn-primary"><i class="fa fa-times"></i> <spring:message code="rest.deletemethods.button.cancel"/></a>
        </form:form>
    </c:when>
    <c:otherwise>
        <spring:message code="rest.deletemethods.label.nomethods"/>
        <p>
        <a href="<c:url value="/web/rest/project/${restProjectId}"/>" class="btn btn-primary"><i class="fa fa-times"></i> <spring:message code="rest.deletemethods.button.cancel"/></a>
        </p>
    </c:otherwise>
</c:choose>
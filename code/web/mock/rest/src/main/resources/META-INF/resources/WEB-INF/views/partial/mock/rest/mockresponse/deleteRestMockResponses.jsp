        ...
        <c:choose>
            <c:when test="${restMockResponses.size() > 0}">
                ...
                <form:form action="${delete_rest_mock_responses_url}" method="POST" modelAttribute="deleteRestMockResponsesCommand">
                    ...
                    <button class="btn btn-danger" type="submit"><i class="fas fa-trash"></i> <span><spring:message code="rest.deleterestmockresponses.button.deleteresponses"/></span></button>
                    <a href="<c:url value="/web/rest/project/${restProjectId}/application/${restApplicationId}/resource/${restResourceId}/method/${restMethodId}"/>" class="btn btn-primary"><i class="fas fa-times"></i> <spring:message code="rest.deleterestmockresponses.button.cancel"/></a>
                </form:form>
            </c:when>
            <c:otherwise>
